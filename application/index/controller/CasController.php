...
class casController extends BaseController {
	...
	public function detail($id) {
		$model = db('cas')->where('id', $id)->find($id);
		if (!$model) {
			$this->error('您要查看的数据不存在！');
		}

		$replys = db('cas_replys')->where('aid', $id)->order('add_time desc')->select();
	...
	public function reply() {
		...
		$data['ip'] = request()->ip();
		$result = db('cas_replys')->insertGetId($data);
		if ($result) {
			$reply = db('cas_replys')->find($result);
	...
	public function zan($id) {
		...
		if (request()->isAjax()) {
			...
			$zan = db('cas_zan')->where('aid', $id)->where('open_id', session('openid'))->find();
			...
			$data['aid'] = $id;
			$data['add_time'] = now_time();
			$result = db('cas')->where('id', $id)->setInc('praise');
			$result2 = db('cas_zan')->insert($data);
			if ($result && $result2) {
				$this->success('点赞成功');
			} else {
				$this->error('点赞失败');
